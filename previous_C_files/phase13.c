 ...
 * 64x64 using software-controlled 32x32 tiling.
 *
 * PL accelerator still computes one 32x32 tile multiply:
 *   C_tile = A_tile x B_tile
 *
 * PS accumulates partial C_tile results into full C_hw.
 */
...
#define INPUT_WORDS      (TILE_WORDS * 2)
...
int run_one_tile(XAxiDma *dma, int ti, int tj, int tk)
{
    int status;
    int timeout;
    ...
    if (status != XST_SUCCESS) {
        xil_printf("FAIL: S2MM start ti=%d tj=%d tk=%d status=%d\r\n",
                   ti, tj, tk, status);
        return -1;
    }
    ...
    if (status != XST_SUCCESS) {
        xil_printf("FAIL: MM2S start ti=%d tj=%d tk=%d status=%d\r\n",
                   ti, tj, tk, status);
        return -1;
    }
    ...
    if (timeout <= 0) {
        xil_printf("FAIL: MM2S timeout ti=%d tj=%d tk=%d STATUS=0x%08x\r\n",
                   ti, tj, tk, Xil_In32(ACCEL_BASE + REG_STATUS));
        return -1;
    }
    ...
    if (timeout <= 0) {
        xil_printf("FAIL: S2MM timeout ti=%d tj=%d tk=%d STATUS=0x%08x\r\n",
                   ti, tj, tk, Xil_In32(ACCEL_BASE + REG_STATUS));
        return -1;
    }
    ...
    if (timeout <= 0) {
        xil_printf("FAIL: accelerator timeout ti=%d tj=%d tk=%d STATUS=0x%08x\r\n",
                   ti, tj, tk, Xil_In32(ACCEL_BASE + REG_STATUS));
        return -1;
    }
...
int main()
{
    ...
    for (int ti = 0; ti < num_tiles; ti++) {
        for (int tj = 0; tj < num_tiles; tj++) {
            for (int tk = 0; tk < num_tiles; tk++) {
                ...
                xil_printf("Tile ti=%d tj=%d tk=%d\r\n", ti, tj, tk);
                ...
                pack_i8(A_tile, input_tile, TILE_BYTES);
                pack_i8(B_tile, &input_tile[TILE_WORDS], TILE_BYTES);

                for (int i = 0; i < OUTPUT_WORDS; i++) {
                    output_tile[i] = 0;
                }

                if (run_one_tile(&dma, ti, tj, tk) != 0) {
                    cleanup_platform();
                    return -1;
                }
    ...
    if (errors == 0) {
        xil_printf("TILED MATRIX MULTIPLY %dx%d PASS\r\n",
                   FULL_SIZE, FULL_SIZE);
    } else {
        xil_printf("TILED MATRIX MULTIPLY %dx%d FAIL: %d errors\r\n",
