/*
 * Phase 11: 8-PE Matrix Multiply Test
 * Same DMA flow, but PL uses 8 parallel PEs.
 * N must be multiple of 8.
 * Change TEST_SIZE to 8, 16, or 32.
 */
#include <stdio.h>
#include <stdint.h>
#include "xaxidma.h"
...
int8_t A_mat[M_DIM*K_DIM];
int8_t B_mat[K_DIM*N_DIM];
...
void pack_i8(const int8_t *s, u32 *d, int n) {
    for (int w=0; w<(n+3)/4; w++) {
        u32 v=0;
        for (int b=0; b<4; b++) { int i=w*4+b; if(i<n) v|=((u32)(uint8_t)s[i])<<(b*8); }
        d[w]=v;
    }
}
...
int main() {
    init_platform();
    xil_printf("8-PE Matrix Multiply Test (%dx%d)\r\n", TEST_SIZE, TEST_SIZE);
    if (N_DIM%8!=0) { xil_printf("FAIL: N must be multiple of 8\r\n"); return -1; }
    ...
    for (int i=0;i<M_DIM;i++)
        for (int j=0;j<N_DIM;j++) {
            int32_t s=0;
            for (int k=0;k<K_DIM;k++) s+=(int32_t)A_mat[i*K_DIM+k]*(int32_t)B_mat[k*N_DIM+j];
            C_sw[i*N_DIM+j]=s;
        }
    ...
    if (errors==0) xil_printf("8-PE MATRIX MULTIPLY PASS\r\n");
    else xil_printf("8-PE MATRIX MULTIPLY FAIL: %d errors\r\n",errors);
    cleanup_platform();
    return 0;
}
