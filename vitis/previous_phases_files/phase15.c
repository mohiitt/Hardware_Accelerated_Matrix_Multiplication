/*
 * Phase 15: Pre-packed tile-order input layout
 *
 * Hardware unchanged from Phase 14.
 *
 * Improvement:
 *   Tile extraction/packing is moved outside the timed hardware loop.
 *   Each output tile has one contiguous packed input stream:
 *
 *   [A(ti,0), B(0,tj), A(ti,1), B(1,tj)]
 *
 * PL still accumulates K tiles internally.
 */
...
u32 input_streams[NUM_OUT_TILES][INPUT_WORDS] __attribute__((aligned(64)));
    ...
    for (int ti = 0; ti < NUM_TILES; ti++) {
        for (int tj = 0; tj < NUM_TILES; tj++) {
            ...
            for (int tk = 0; tk < NUM_KTILES; tk++) {
                ...
                pack_i8(B_tile,
                        &input_streams[out_tile_idx][base + TILE_WORDS],
                        TILE_BYTES);
            }
        }
    }
...
int run_one_output_tile(XAxiDma *dma, u32 *input_stream, int ti, int tj)
{
    int status;
    int timeout;
...
int main()
{
    ...
    xil_printf("Pre-packed tile-order test (%dx%d, tile=%d)\r\n",
    ...
    xil_printf("Pre-packing tile streams...\r\n");
    ...
    xil_printf("Running hardware on pre-packed streams...\r\n");
    ...
    xil_printf("Hardware time, pre-packed input: %d us\r\n",
    ...
    if (errors == 0) {
        xil_printf("PRE-PACKED TILE INPUT %dx%d PASS\r\n",
                   FULL_SIZE, FULL_SIZE);
    } else {
        xil_printf("PRE-PACKED TILE INPUT %dx%d FAIL: %d errors\r\n",
